...
void OnConnect(struct mosquitto *mosq, void *obj, int reason_code)
{
	...
	/* Making subscriptions in the OnConnect() callback means that if the
...
void OnSubscribe(struct mosquitto *mosq, void *obj, int mid, int qos_count, const int *granted_qos)
{
	...
	for(i=0; i<qos_count; i++){
		printf("OnSubscribe: %d:granted qos = %d\n", i, granted_qos[i]);
		if(granted_qos[i] <= 2){
...
int main(int argc, char *argv[])
{
	...
	mosquitto_subscribe_callback_set(pMosq, OnSubscribe);
	mosquitto_message_callback_set(pMosq, OnMessage);

...
int Init_SDL_ctx(SDL_Window* &pWindow, SDL_Renderer* &pRenderer) {
  ...
  pWindow = SDL_CreateWindow( "SDL2_overlay", 
...
int main(int argc, char *argv[]) {
  ...
  while (!bQuit) {
    ...
    // draw text
    DrawText(pTxtTexture,pFont,
        std::string("q : exit, w/s : change opacity , opacity =") +std::to_string(fOpaque) + std::string(" fps: ")+std::to_string((int)(dbActualFPS)),pRenderer);

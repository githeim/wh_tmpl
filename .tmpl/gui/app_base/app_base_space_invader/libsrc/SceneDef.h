...
struct SceneRuntime {
  ...
  bool phaseScreenPresented = false;
  bool phaseWorkDone = false;
  bool exitAfterFinishing = false;
  bool shouldQuit = false;
  std::optional<SceneId> prevScene;
  std::string lifecycleNote = "Application booted";
  std::map<SceneId, int> enterCounts;
  std::map<SceneId, int> exitCounts;
 ...
 */
struct GameState {
  int score        = 0;      ///< 게임 점수 (적기 격추 시 증가)
  int playerLives  = 3;      ///< 플레이어 남은 목숨
};
